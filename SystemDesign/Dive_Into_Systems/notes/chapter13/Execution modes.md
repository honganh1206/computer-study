...
1. User mode / Ring 3 (When user-level programs run on the CPU): The CPU only executes *user-level instructions* and accesses memory locations the OS makes available. The CPU cannot access the OS's instructions and data.
2. Kernel mode / Ring 0 (When OS code runs on the CPU): A CPU can execute *any* instructions and access *any* memory location. The CPU can access hardware components and execute special instructions.
