...
void countElems(int *counts, int *array, long length) {
    ...
    // Set the default size of the thread team
    // Also implicitly create threads at the start of the block
    // and join them at the end of the block
    // shared means the arguments are shared (global) among threads
    #pragma omp parallel default(none) shared(counts, array, length)
    {
        int val, i, local[MAX] = {0};
        // Parallelize the for loop, split iterations among threads
        #pragma omp for
        for (i = 0; i < length; i++){
            val = array[i];
            local[val]++;
        ...
        // Code in this scope should be executed by exactly one thread at a time
       #pragma omp critical
       {
           for (i = 0; i < MAX; i++){
               counts[i] += local[i];
...
void writeArray(int *counts, int *array) {
    ...
    // Generate a team of threads and parallelize the for loop.
    // Each thread must complete the iteration before being assigned a new iteration.
    #pragma omp parallel for schedule(dynamic)
    for (i = 0; i < MAX; i++) {
        int j = 0, amt, start = 0;
        // Each thread calculates its unique start value
        // based on the sum of all previous elements in counts?
